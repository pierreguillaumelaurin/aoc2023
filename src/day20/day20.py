...
def part_one(raw_communications: List[str]) -> List[Pulse]:
    ...
    def to_module(raw_module: str) -> Module:
        if raw_module == "broadcaster":
            return {"id": "broadcaster", "type": ModuleType.BROADCAST}
        elif raw_module[0] == "%":
            return {"id": raw_module[1:], "type": ModuleType.FLIPFLOP, "on": False}
        else:
            return {"id": raw_module[1:], "type": ModuleType.CONJUNCTION, "on": False}
    ...
    return [
        {
            "sender": {"id": "button", "type": "button"},
            "pulse": Pulse.LOW,
            "receivers": ["broadcaster"],
        }
    ] + [tokenize(raw_communication) for raw_communication in raw_communications]
